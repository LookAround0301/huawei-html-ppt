 ...
 */
import { findPageElement, hasPageSelector } from "./pages.js";
/**
 ...
 */
export function applyDiagramOperations(xmlContent, operations, pageSelector) {
    const errors = [];
    // Parse the XML
    ...
    if (doc.documentElement?.tagName === "mxfile") {
        ...
        if (!root) {
            const pageId = found.element.getAttribute("id") || `(index ${found.index})`;
            return {
                result: xmlContent,
                errors: [
                    {
                ...
                errors: [
                    {
                        type: "update",
                        cellId: "",
                        message: "Page selector provided but document is not multi-page (no <mxfile> wrapper). Use create_new_diagram with a full <mxfile> first, or omit the page selector.",
                    },
                ],
            };
        }
