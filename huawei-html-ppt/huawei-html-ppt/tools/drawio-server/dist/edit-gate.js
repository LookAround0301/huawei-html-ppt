...
function canonicalizeElement(el) {
    ...
    for (const child of Array.from(el.childNodes)) {
        if (child.nodeType === 1) {
            children += canonicalizeElement(child);
        }
        else if (child.nodeType === 3 || child.nodeType === 4) {
            const text = (child.textContent ?? "").trim();
            if (text)
                children += JSON.stringify(text);
        }
    }
 ...
 * any real page name).
 */
export function contentFingerprint(xml, includeNames = true) {
...
export function checkEditGate(lastSeenXml, liveXml) {
    ...
    if (liveXml && liveXml !== lastSeenXml) {
        ...
        const includeNames = !isMxGraphModel(liveXml) && !isMxGraphModel(lastSeenXml);
        if (contentFingerprint(liveXml, includeNames) !==
