...
export function hasPageSelector(s) {
    if (!s)
        return false;
    return (Boolean(s.page_id) || Boolean(s.page_name) || s.page_index !== undefined);
}
/**
 ...
 */
export function normalizeToMxfile(xml, opts = {}) {
 ...
 */
export function parseMxfile(xml) {
    try {
...
export function serializeMxfile(doc) {
 ...
 */
export function projectPage(xml, selector) {
 ...
 */
export function findPageElement(doc, selector) {
 ...
 * shape is wrong.
 */
export function addPageToDoc(doc, opts = {}) {
    ...
    if (opts.xml?.trim()) {
        ...
        if (isMxFile(trimmed)) {
            throw new Error("addPageToDoc: opts.xml must be a bare <mxGraphModel>; received a full <mxfile>. Extract the target diagram's <mxGraphModel> first.");
        }
        if (!isMxGraphModel(trimmed)) {
            throw new Error("addPageToDoc: opts.xml must be a bare <mxGraphModel>.");
        }
        inner = trimmed;
    }
    else {
    ...
    if (tempDoc.querySelector("parsererror")) {
        throw new Error("Failed to parse new page xml — make sure it is a valid <mxGraphModel>");
    }
    const newDiagram = tempDoc.querySelector("diagram");
    if (!newDiagram) {
        throw new Error("Failed to construct <diagram> element for new page");
    }
    const imported = doc.importNode(newDiagram, true);
...
export function renamePageInDoc(doc, selector, newName) {
    const found = findPageElement(doc, selector);
    if (!found)
        return false;
 ...
 */
export function deletePageFromDoc(doc, selector) {
