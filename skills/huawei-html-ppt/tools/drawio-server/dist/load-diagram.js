...
import { isMxFile, isMxGraphModel, normalizeToMxfile, parseMxfile, serializeMxfile, } from "./pages.js";
/**
 * Decode one compressed page body (base64 → raw deflate → URI-decode).
 * Returns null if the text isn't in that format.
 */
export function decompressPageContent(compressed) {
    try {
        const inflated = inflateRawSync(Buffer.from(compressed.trim(), "base64")).toString("utf-8");
        try {
 ...
 */
export function parseDrawioFileContent(content) {
    ...
    if (!doc)
        return { ok: false, error: "Failed to parse <mxfile> XML." };
    let decompressedAny = false;
    for (const d of Array.from(doc.querySelectorAll("diagram"))) {
        if (d.querySelector("mxGraphModel"))
            continue;
        const text = (d.textContent || "").trim();
        if (!text)
            continue; // an empty page is valid
        const pageLabel = d.getAttribute("name") || d.getAttribute("id") || "unnamed";
...
//# sourceMappingURL=load-diagram.js.map