...
import { getState, requestExport, requestSync, setState, shutdown, startHttpServer, waitForSync, } from "./http-server.js";
import { parseDrawioFileContent } from "./load-diagram.js";
import { log } from "./logger.js";
import { addPageToDoc, deletePageFromDoc, hasPageSelector, listPagesFromDoc, normalizeToMxfile, parseMxfile, projectPage, renamePageInDoc, serializeMxfile, } from "./pages.js";
...
const pageSelectorSchema = {
    page_id: z
        ...
        .min(1)
        .optional()
        .describe("Target a page by its id (as returned by list_pages or add_page). Wins over page_name and page_index when multiple are set."),
    page_name: z
        .string()
        .min(1)
        .optional()
        .describe('Target a page by its display name (e.g. "CNN"). Used only when page_id is not set.'),
    page_index: z
        .number()
        .int()
        .nonnegative()
        .optional()
        .describe("Target a page by its 0-based tab index. Used only when page_id and page_name are not set."),
};
/**
...
function pickPageSelector(input) {
    const selector = {};
    if (input.page_id)
        selector.page_id = input.page_id;
    if (input.page_name)
        selector.page_name = input.page_name;
    if (input.page_index !== undefined)
        selector.page_index = input.page_index;
    return selector;
}
...
function describeSelector(s) {
    if (s.page_id)
        return `id="${s.page_id}"`;
    if (s.page_name)
        return `name="${s.page_name}"`;
    if (s.page_index !== undefined)
        return `index=${s.page_index}`;
    return "first page";
}
...
- Text: fontSize=14; fontStyle=1 (bold); align=center`,
    inputSchema: {
        xml: z
            .string()
            .describe("REQUIRED: Either a complete <mxGraphModel> (legacy single-page) or a full <mxfile> with one or more <diagram> children (multi-page)."),
    },
}, async ({ xml: inputXml }) => {
    try {
        if (!currentSession) {
...
server.registerTool("load_diagram", {
    description: "Load a .drawio file from disk into the current session, REPLACING the entire diagram (all pages). " +
    ...
    inputSchema: {
        path: z
            .string()
            .describe("Path to the .drawio file to load (e.g., ./diagram.drawio)"),
    },
}, async ({ path }) => {
    try {
        if (!currentSession) {
        ...
        log.info(`Loading diagram from ${absolutePath} (${xml.length} chars)`);
...
server.registerTool("edit_diagram", {
    description: "Edit a specific page in the current diagram by ID-based operations (update/add/delete cells).\n\n" +
    ...
    try {
        ...
        const gate = checkEditGate(currentSession.lastSeenXml, browserState?.xml ?? "");
        if (!gate.ok) {
            log.warn(gate.reason === "stale"
        ...
        log.info(`Editing diagram with ${operations.length} operation(s) on ${describeSelector(pageSelector)}`);
        ...
        const { result, errors } = applyDiagramOperations(currentSession.xml, validatedOps, pageSelector);
        if (errors.length > 0) {
...
server.registerTool("export_diagram", {
    ...
    try {
        ...
        requestExport(currentSession.id, detectedFormat, projectionXml);
 ...
 */
async function loadMxfileForMutation() {
    if (!currentSession) {
        return {
            ok: false,
    ...
    if (!currentSession.xml) {
        return {
            ok: false,
            message: "No diagram exists yet. Use create_new_diagram first, then page tools.",
        };
    }
    ...
    if (!normalized) {
        return {
            ok: false,
            message: "Current session XML is neither <mxGraphModel> nor <mxfile>; cannot perform page operations.",
        };
    }
...
server.registerTool("list_pages", {
    description: "List every page (tab) in the current diagram. Returns each page's id, name, 0-based index, and cell count. Use this to discover what pages exist before targeting one with edit_diagram, get_diagram, export_diagram, rename_page, or delete_page.",
    inputSchema: {},
}, async () => {
    try {
        ...
        const lines = pages.map((p) => `  [${p.index}] id="${p.id}" name="${p.name}" cells=${p.cellCount}`);
        return {
            content: [
                {
                    type: "text",
...
server.registerTool("add_page", {
    description: 'Append a new page (tab) to the current diagram WITHOUT touching existing pages or unsaved user changes. Use this when the user wants "another diagram alongside" — e.g. "add a CNN page" — instead of create_new_diagram which wipes everything.\n\n' +
        "Inputs:\n" +
    ...
    inputSchema: {
        name: z
            .string()
            .optional()
            .describe('Optional display name for the new tab (e.g. "CNN"). Defaults to "Page-N".'),
        id: z
            .string()
            .min(1)
            .optional()
            .describe("Optional explicit page id. If omitted the server generates one. Must be unique across pages."),
        xml: z
            .string()
            .optional()
            .describe('Optional starting <mxGraphModel> XML for the new page. Must include <root> with id="0" and id="1" cells. If omitted the page starts blank.'),
    },
}, async (input) => {
    ...
    try {
        ...
        let cleanXml = xml;
        if (cleanXml) {
            const { valid, error, fixed, fixes } = validateAndFixXml(cleanXml);
            if (fixed) {
                cleanXml = fixed;
                log.info(`add_page: starting XML auto-fixed: ${fixes.join(", ")}`);
            }
            if (!valid && error) {
                return {
                    content: [
        ...
        log.info(`Added page id=${info.id} name="${info.name}" index=${info.index}`);
        return {
            content: [
                {
                    type: "text",
...
server.registerTool("rename_page", {
    description: "Rename an existing page (tab). At least one of page_id / page_name / page_index is required to identify which page to rename. The new_name becomes the visible tab label in the editor.",
    inputSchema: {
        ...pageSelectorSchema,
        new_name: z
            .string()
    ...
    try {
        ...
        loaded.writeBack(loaded.doc);
        log.info(`Renamed page ${describeSelector(pageSelector)} → "${new_name}"`);
        return {
            content: [
                {
                    type: "text",
...
server.registerTool("delete_page", {
    description: "Delete a page (tab) from the current diagram. At least one of page_id / page_name / page_index is required. Refuses to delete the last remaining page — the editor needs at least one tab.",
    inputSchema: {
        ...pageSelectorSchema,
    },
}, async (input) => {
    ...
    try {
        ...
        loaded.writeBack(loaded.doc);
        log.info(`Deleted page id=${outcome.deletedId} index=${outcome.deletedIndex}`);
        return {
            content: [
                {
                    type: "text",
